...
func TestLessThanName(t *testing.T) {
	mut := LessThan{}

	if got, want := mut.Name(), "ConditionalBoundary_LSS_LEQ"; got != want {
		t.Fatalf("Name() = %q, want %q", got, want)
	}
}

func TestLessThanCanMutate(t *testing.T) {
	mut := LessThan{}
	...
	tests := []struct {
		...
		{
			name: "binary LSS is mutable",
			node: &ast.BinaryExpr{Op: token.LSS},
			want: true,
		},
		{
			name: "binary LEQ is not mutable",
...
func TestLessThanMutate(t *testing.T) {
	mut := LessThan{}
	...
	if mutatedExpr.Op != token.LEQ {
		t.Fatalf("Mutate() Op = %v, want token.LEQ", mutatedExpr.Op)
	}
