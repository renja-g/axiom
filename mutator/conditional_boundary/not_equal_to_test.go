...
func TestNotEqualToName(t *testing.T) {
	mut := NotEqualTo{}

	if got, want := mut.Name(), "ConditionalBoundary_NEQ_EQL"; got != want {
		t.Fatalf("Name() = %q, want %q", got, want)
	}
}

func TestNotEqualToCanMutate(t *testing.T) {
	mut := NotEqualTo{}
	...
	tests := []struct {
		...
		{
			name: "binary NEQ is mutable",
			node: &ast.BinaryExpr{Op: token.NEQ},
			want: true,
		},
		{
			name: "binary EQL is not mutable",
...
func TestNotEqualToMutate(t *testing.T) {
	mut := NotEqualTo{}
	...
	if mutatedExpr.Op != token.EQL {
		t.Fatalf("Mutate() Op = %v, want token.EQL", mutatedExpr.Op)
	}
