...
func TestDivisionName(t *testing.T) {
	mut := Division{}
...
func TestDivisionCanMutate(t *testing.T) {
	mut := Division{}
	...
	tests := []struct {
		...
		{
			name: "binary division is mutable",
			node: &ast.BinaryExpr{Op: token.QUO},
			want: true,
		},
		{
			name: "binary multiplication is not mutable",
...
func TestDivisionMutate(t *testing.T) {
	mut := Division{}
	...
	if mutatedExpr.Op != token.MUL {
		t.Fatalf("Mutate() Op = %v, want token.MUL", mutatedExpr.Op)
	}
