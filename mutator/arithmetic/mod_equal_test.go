...
func TestModEqualName(t *testing.T) {
	mut := ModEqual{}

	if got, want := mut.Name(), "Arithmetic_REM_ASSIGN"; got != want {
		t.Fatalf("Name() = %q, want %q", got, want)
	}
}

func TestModEqualCanMutate(t *testing.T) {
	mut := ModEqual{}
	...
	tests := []struct {
		...
		{
			name: "modulo assignment is mutable",
			node: &ast.AssignStmt{Tok: token.REM_ASSIGN},
			want: true,
		},
		{
...
func TestModEqualMutate(t *testing.T) {
	mut := ModEqual{}
