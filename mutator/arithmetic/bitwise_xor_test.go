...
func TestBitwiseXorName(t *testing.T) {
	mut := BitwiseXor{}

	if got, want := mut.Name(), "Arithmetic_XOR"; got != want {
		t.Fatalf("Name() = %q, want %q", got, want)
	}
}

func TestBitwiseXorCanMutate(t *testing.T) {
	mut := BitwiseXor{}
	...
	tests := []struct {
		...
		{
			name: "binary XOR is mutable",
			node: &ast.BinaryExpr{Op: token.XOR},
			want: true,
		},
		{
...
func TestBitwiseXorMutate(t *testing.T) {
	mut := BitwiseXor{}
