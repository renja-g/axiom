...
func TestMultiplicationName(t *testing.T) {
	mut := Multiplication{}

	if got, want := mut.Name(), "Arithmetic_QUO"; got != want {
		t.Fatalf("Name() = %q, want %q", got, want)
	}
}

func TestMultiplicationCanMutate(t *testing.T) {
	mut := Multiplication{}
	...
	tests := []struct {
		...
		{
			name: "binary multiplication is mutable",
			node: &ast.BinaryExpr{Op: token.MUL},
			want: true,
		},
		{
			name: "binary division is not mutable",
			node: &ast.BinaryExpr{Op: token.QUO},
			want: false,
		},
		{
...
func TestMultiplicationMutate(t *testing.T) {
	mut := Multiplication{}

	original := &ast.BinaryExpr{
		Op: token.MUL,
		X:  &ast.BasicLit{Kind: token.INT, Value: "6"},
	...
	if mutatedExpr.Op != token.QUO {
		t.Fatalf("Mutate() Op = %v, want token.QUO", mutatedExpr.Op)
	}
